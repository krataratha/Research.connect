...
const EditProfileModal = ({ isOpen, onClose, initialData, onSaveSuccess }) => {
 ...
 const [success, setSuccess] = useState('');   // New: Stores success message to show after successful save
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (initialData.researchAreas) {
        setAreasInput(
          initialData.researchAreas
            .map((a) => a.researchArea?.areaName)
            .filter(Boolean)
            .join(', ')
        );
      }
  ...
  const handleProfileSave = async () => {
    ...
    try {
      ...
      // Success case
      setSuccess('✅ Profile updated successfully!');   // Show success message to user
  ...
  const handleSaveResearch = async () => {
    try {
      const researchAreas = areasInput
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean);
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white border border-slate-200 shadow-2xl rounded-3xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Customize your personal bio, employment status, academic history, and external
              identities.
            </p>
          </div>
          <button
        ...
        <div className="p-6 flex-1 overflow-y-auto text-left space-y-6">
          ...
          {/* ==================== SUCCESS MESSAGE ==================== */}
          {/* Added to give user clear positive feedback after saving */}
          {success && (
            <div className="p-4 bg-green-50 border border-green-100 rounded-2xl text-xs text-green-600 font-medium flex justify-between items-center">
              <span>{success}</span>
              <button
                onClick={() => setSuccess('')}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className="px-4 py-2 border border-slate-200 rounded-xl text-sm"
                  placeholder="e.g. AI Researcher | Postdoctoral Fellow at Stanford"
                />
              </div>
              <div className="flex flex-col gap-1.5 md:col-span-2">
                ...
                <textarea
                  ...
                  rows={3}
                  className="px-4 py-2 border border-slate-200 rounded-xl text-sm"
                  placeholder="Tell other researchers about your research focuses, lab work, or academic achievements..."
                />
              </div>
              <div className="flex flex-col gap-1.5">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className="px-4 py-2 border border-slate-200 rounded-xl text-sm"
                  placeholder="https://sarahjenkins.lab.stanford.edu"
                />
              </div>
              <div className="flex flex-col gap-1.5">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                {educationList.length === 0 ? (
                  <p className="text-xs text-slate-400">
                    No education entries added yet. Fill the form below to add one.
                  </p>
                ) : (
                      ...
                      <div key={edu._id} className="p-4 flex items-center justify-between gap-4">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            {edu.university} • {edu.startYear} - {edu.endYear || 'Present'}
                          </p>
                        </div>
                        <button
              ...
              <div className="border border-blue-100 rounded-3xl p-5 bg-gradient-to-r from-blue-50/20 to-indigo-50/20 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-left">
                  ...
                  <input
                    type="text"
                    placeholder="University (e.g. MIT)"
                  ...
                  <input
                    type="text"
                    placeholder="Field of Study (e.g. Computational Genetics)"
                    value={newEdu.fieldOfStudy}
                    onChange={(e) =>
                      setNewEdu((prev) => ({ ...prev, fieldOfStudy: e.target.value }))
                    }
                    className="px-4 py-2 border border-slate-200 rounded-xl text-sm bg-white"
                  />
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setNewEdu((prev) => ({ ...prev, startYear: parseInt(e.target.value) }))
                      }
                      className="px-4 py-2 border border-slate-200 rounded-xl text-sm bg-white"
                    />
                    <input
                      ...
                      onChange={(e) =>
                        setNewEdu((prev) => ({ ...prev, endYear: parseInt(e.target.value) }))
                      }
                      className="px-4 py-2 border border-slate-200 rounded-xl text-sm bg-white"
                    />
                  </div>
              ...
              <div className="border border-indigo-100 rounded-3xl p-5 bg-gradient-to-r from-indigo-50/20 to-purple-50/20 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-left">
                  <input
                    ...
                    onChange={(e) =>
                      setNewExp((prev) => ({ ...prev, organization: e.target.value }))
                    }
                    className="px-4 py-2 border border-slate-200 rounded-xl text-sm bg-white"
                  />
                  <input
                    type="text"
                    placeholder="Role (e.g. Senior Researcher)"
                    value={newExp.role}
                  ...
                  <input
                    ...
                    onChange={(e) => setNewExp((prev) => ({ ...prev, department: e.target.value }))}
                    className="px-4 py-2 border border-slate-200 rounded-xl text-sm bg-white"
                  />
                  <select
                    value={newExp.employmentType}
                    onChange={(e) =>
                      setNewExp((prev) => ({ ...prev, employmentType: e.target.value }))
                    }
                    className="px-4 py-2 border border-slate-200 rounded-xl text-sm bg-white"
                  >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setNewExp((prev) => ({ ...prev, startYear: parseInt(e.target.value) }))
                      }
                      className="px-4 py-2 border border-slate-200 rounded-xl text-sm bg-white"
                    />
                    <input
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 border border-slate-100 rounded-2xl flex items-center justify-between gap-4">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-slate-400">
                    Synchronize employment, awards, and works.
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="px-4 py-2 border border-slate-200 rounded-xl text-xs w-48 bg-slate-50"
                  />
                  <button
                    onClick={() => handleLinkIdentity('orcid')}
                    className="px-3 py-2 bg-slate-800 text-white rounded-xl text-xs font-bold flex items-center gap-1 cursor-pointer hover:bg-slate-700"
                  >
              ...
              <div className="p-4 border border-slate-100 rounded-2xl flex items-center justify-between gap-4">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-slate-400">
                    Link profile summary, experience, and education details.
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <input
                  ...
                  />
                  <button
                    onClick={() => handleLinkIdentity('linkedin')}
                    className="px-3 py-2 bg-slate-800 text-white rounded-xl text-xs font-bold flex items-center gap-1 cursor-pointer hover:bg-slate-700"
                  >
                    <Link2 className="w-3.5 h-3.5" /> Link
                  </button>
              ...
              <div className="p-4 border border-slate-100 rounded-2xl flex items-center justify-between gap-4">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-slate-400">
                    Lock citation counts and Scopus research scores.
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <input
                    type="text"
                    placeholder="57204859344"
                    value={scopusInput}
                    onChange={(e) => setScopusInput(e.target.value)}
                    className="px-4 py-2 border border-slate-200 rounded-xl text-xs w-48 bg-slate-50"
                  />
                  <button
                    onClick={() => handleLinkIdentity('scopus')}
                    className="px-3 py-2 bg-slate-800 text-white rounded-xl text-xs font-bold flex items-center gap-1 cursor-pointer hover:bg-slate-700"
                  >
                    <Link2 className="w-3.5 h-3.5" /> Link
                  </button>
                      ...
                      >
                        <div className="min-w-0 flex-1 text-left">
                          ...
                          <p className="text-xs text-slate-500 truncate">
                            {pub.journal || pub.publisher || 'Unknown Venue'} •{' '}
                            {pub.publicationYear}
                          </p>
                        </div>
                        <button
              ...
              <div className="border border-blue-100 rounded-3xl p-5 bg-gradient-to-r from-blue-50/20 to-indigo-50/20 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-left">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setNewPub((prev) => ({
                          ...prev,
                          publicationYear: parseInt(e.target.value) || 2026,
                        }))
                      }
                      className="px-4 py-2 border border-slate-200 rounded-xl text-sm bg-white"
                    />
                    <select
                      ...
                      onChange={(e) =>
                        setNewPub((prev) => ({ ...prev, publicationType: e.target.value }))
                      }
                      className="px-4 py-2 border border-slate-200 rounded-xl text-sm bg-white"
                    >
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewPub((prev) => ({ ...prev, publicationUrl: e.target.value }))
                    }
                    className="px-4 py-2 border border-slate-200 rounded-xl text-sm bg-white"
                  />
                  <input
          ...
          {activeSubTab === 'research' && (
            <div className="space-y-6 text-left">
              ...
              <p className="text-xs text-slate-400">
                Add keywords and research areas separated by commas to customize your profile's
                focus.
              </p>

              <div className="flex flex-col gap-2">
                <label className="text-xs font-bold text-slate-700">
                  Research Areas (e.g. Machine Learning, Biostatistics, Cardiology)
                </label>
                <input
                  type="text"
                  value={areasInput}
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs font-bold text-slate-700">
                  Keywords (e.g. deep learning, transformer, federated learning)
                </label>
                <input
                  type="text"
                  value={keywordsInput}
                  ...
                  {profileHistory.map((hist) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-slate-500">
                          {hist.changeSummary} • {new Date(hist.createdAt).toLocaleString()}
                        </p>
                      </div>
                      <button
