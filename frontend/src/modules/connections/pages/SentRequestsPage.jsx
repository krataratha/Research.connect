...
const SentRequestsPage = () => {
  ...
  const { data: requests, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await connectionsService.getSentRequests();
      return res.data || [];
    }
  });
    ...
    <div className="space-y-6 text-left">
      {requests && requests.length > 0 ? (
        ...
        <div className="text-center py-16 bg-white border border-slate-200 rounded-3xl shadow-sm space-y-3">
          <Send className="w-10 h-10 text-slate-400 mx-auto opacity-50" />
          <h4 className="text-sm font-black text-slate-900 uppercase">No Sent Invitations</h4>
          <p className="text-xs text-[#475569] max-w-xs mx-auto font-semibold text-center">
            You don't have any sent connection requests that are currently pending.
          </p>
        </div>
      )}
    </div>
