...
const FollowingPage = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['following', username, debouncedSearch],
    queryFn: async () => {
      const res = await followService.getFollowing(username, {
        limit: 12,
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      if (res.success) {
        setFollowingList((prev) => [...prev, ...res.data.docs]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white border border-slate-200 p-5 rounded-3xl shadow-[0_4px_20px_rgba(15,23,42,0.02)]">
        <div>
          ...
          <p className="text-[10px] text-[#475569] font-bold uppercase tracking-wider mt-0.5">Researchers followed by this profile</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          {hasNextPage && (
            ...
            >
              {isFetchingMore ? 'Loading...' : 'Load More Following'}
            </button>
          )}
        </div>
      ) : (
        <div className="text-center py-16 bg-white border border-slate-200 rounded-3xl shadow-sm space-y-3">
          ...
          <h4 className="text-sm font-black text-slate-900 uppercase">No Following Found</h4>
          <p className="text-xs text-[#475569] max-w-xs mx-auto font-semibold">
            {debouncedSearch ? 'No matching researchers. Try another search.' : 'This researcher is not following anyone yet.'}
          </p>
        </div>
      )}
    </div>
